...
function create_semester() {
    ...
    if [ "$create_semester_value_exsist" != "$semester" ]; then
...
function create_course() {
    ...
    if [ $reaturn_value_teacher != 0 ]; then
        if [ $reaturn_value_semester != 0 ]; then
...
function modify_teacher() {
    ...
    if [ $reaturn_value_teacher != 0 ]; then
        ...
        while read course_id course_name semester teacher_id; do
...
function delete_student() {
    ...
    if [ $reaturn_value_delete_student != 0 ]; then
        INPUT=student.csv
        count=0
        OLDIFS=$IFS
        IFS=','
        ...
        while read delete_file_s_id delete_file_s_name; do
            if [ $user_delete_student_id != $delete_file_s_id ]; then
...
function return_course_count() {
    ...
    while read course_id student_id semester attendance quiz mid final; do
        if [ $course_id == $1 ]; then
...
function enroll_course() {
    ...
    if [ $semesterExsist == 0 ] || [ $studentExsist == 0 ] || [ $courseExsist == 0 ]; then
...
function view_course_enrollments() {
    ...
    while read course_id student_id semester attendance quiz midterm final; do
...
function teacher_course_enrolled_students() {
    ...
    [ ! -f $INPUTCOURSE ] && {
        echo "$INPUTCOURSE file not found"
        exit
    }
    ...
    while read course_id student_id semester attendance quiz midterm final; do
        ...
        while read course_course_id course_course_name course_semester course_teacher_id; do
            if [ $course_id == $course_course_id ] && [ $semester == $course_semester ] && [ $1 == $course_teacher_id ]; then
                return_function_value student $student_id 2
...
function teacher_course_students_marks() {
    ...
    if [ $update_student_attendance_marks -gt 15 ] || [ $update_student_quiz_marks -gt 15 ] || [ $update_student_mid_marks -gt 30 ] || [ $update_student_final_marks -gt 40 ]; then
    ...
    while read course_id student_id semester attendance quiz midterm final; do
        flag="false"
        while read course_course_id course_course_name course_semester course_teacher_id; do
            if [ $course_id == $teacher_update_course_id ] && [ $teacher_update_student_id == $student_id ] && [ $teacher_update_semester == $semester ] && [ $course_id == $course_course_id ] && [ $semester == $course_semester ] && [ $1 == $course_teacher_id ]; then
...
function view_students_info_admin() {
    ...
    [ ! -f $INPUTCOURSEENROLLMENT ] && {
        echo "$INPUTCOURSEENROLLMENT file not found"
        exit
    }
    [ ! -f $INPUTCOURSE ] && {
        echo "$INPUTCOURSE file not found"
    ...
    while read student_student_id student_student_name; do
        counting=$(expr $counting + 1)

        echo -e "$counting : $student_student_id\t$student_student_name"

        while read en_course_id en_student_id en_semester en_attendance en_quiz en_mid en_final; do
            while read course_course_id course_course_name course_semester course_teacher_id; do
                if [ $student_student_id == $en_student_id ] && [ $en_course_id == $course_course_id ] && [ $en_semester == $course_semester ]; then
...
function view_students_search() {
    ...
    while read en_course_id en_student_id en_semester en_attendance en_quiz en_mid en_final; do
        while read course_course_id course_course_name course_semester course_teacher_id; do
            if [ $user_search_student_id == $en_student_id ] && [ $en_course_id == $course_course_id ] && [ $en_semester == $course_semester ]; then
                return_function_value teacher $course_teacher_id 2
                local file_teacher_name=$function_return_value
...
function view_single_student() {
    ...
    while read en_course_id en_student_id en_semester en_attendance en_quiz en_mid en_final; do
        while read course_course_id course_course_name course_semester course_teacher_id; do
            if [ $1 == $en_student_id ] && [ $en_course_id == $course_course_id ] && [ $en_semester == $course_semester ]; then
                return_function_value teacher $course_teacher_id 2
                local file_teacher_name=$function_return_value
...
while [ $choice == "y" ] || [ $choice == "Y" ]; do
    ...
    2)
        ...
        if [ "$teacher_teacher_exsist_entry" == 0 ]; then
